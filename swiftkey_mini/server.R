...
# Load Quadgram,Trigram & Bigram Data frame files built from the Prediction Model
...
Predict <- function(x) {
    ...
    if (length(xs)>= 3) {
        ...
        else {mesg<<- "Next word is predicted using Trigram."; head(trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3],1)}
    }
    else if (length(xs) == 1){
        xs <- tail(xs,1)
...
## Shiny Server logic function for text prediction 
